...
Security tokens can be granted for specific resources. In this case the user generating the token grants
their permissions but only if the resource path matches the one for the token.  This would be a way
