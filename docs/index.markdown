...
```
If a file called index.htm or index.html is found in the directory, it will be loaded automatically
...
Core Datastore
==============
Data used for the running of the freehold instance are stored in the core datastore located at */v1/datastore/core.ds*. 
By default the permissions on the core datastore are as follows:

* Private - readonly
* Friend - none
* Public - none

The core datastore is designed to only be modified via specific REST requests and not directly. It contains data used
for authentication, authorization, session data, permissions, and settings.

Only the owner of the core datastore can make requests to modify it, and the owner is set by default to the first user
in the system.
...
A *Security Token* is a cryptographically random value that may have a present expiration when it will no
...
Authentication and Session data are stored in the /v1/datastore/core.ds datastore, and accessed via the path
*/v1/auth/*.

### /v1/auth/user

Information on Users in the system.  Stored in the *user* collection.
```
{
	collection: "user",
	key: <username>,
